...
<html>
	<head>
		...
		<title>CRUD MANAGER - FORNECEDORES</title>
	</head>
	<body>
		...
		<div id="container" class="container-fluid">
			...
			<div id="top" class="row">
			    ...
			    <div class="col-md-3">
			        <a href="/crud-manager/supplier/form" class="btn btn-danger pull-right h2"><span class="glyphicon glyphicon-plus" /></span>&nbspAdicionar Fornecedor</a>
			    </div>
		            	...
		            	<tbody>
		            		<c:forEach var="supplier" items="${suppliers}">
			            		<tr>
				                    ...
				                    <td>${supplier.getFone()}</td>
				                    <td>${supplier.getAdress()}</td>
				                    <td>${supplier.getCompany().getName()}</td>				                    
				                    
				                    <td class="actions">
				                        <a class="btn btn-info btn-xs" 
				                           href="${pageContext.request.contextPath}/supplier/update?supplierId=${supplier.getId()}" >
				                    ...
				                    <td class="actions">
				                        <a class="btn btn-danger btn-xs modal-remove"
				                           data-supplier-id="${supplier.getId()}" 
				                           data-supplier-name="${supplier.getName()}" data-toggle="modal" 
		...
		<script type="text/javascript">
			$(document).ready(function() {
			    ...
			    $(".modal-remove").click(function () {
		            ...
		            $(".modal-body #hiddenValue").text("o fornecedor '"+supplierName+"'");
		            $("#id").attr( "value", supplierId);
		            $("#entityName").attr("value", supplierName);
		            $("#form").attr( "action","supplier/delete");
		        })
			});
		</script>
	</body>
